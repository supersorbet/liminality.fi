...
async function printSynthetixPool(App, info, chain="eth", customURLs) {
    ...
    switch (chain) {
      case "eth":
        ...
        break;
      case "optimism":
        _print(`\n    <a target="_blank" href="https://optimistic.etherscan.io/address/${info.stakingAddress}#code">Contract\n</a>`);
        break;
      case "avax":
    ...
    _print(`\n    <a target="_blank" href="${{unstake}}">Unstake</a> ${info.userStaked.toFixed(6)}`);
    _print(`    <a target="_blank" href="${{claim}}">Claim</a> ${info.earned.toFixed(6)} ${info.rewardTokenTicker} ($${formatMoney(info.earned*info.rewardTokenPrice)})\n`);
    if (info.stakeTokenTicker != "ETH") {
      _print(`    <a target="_blank" href="${{revoke}}>Revoke (set approval to 0)`)
    }
    _print(`    <a target="_blank" href="${{exit}}>Exit</a>`)
    _print("");
